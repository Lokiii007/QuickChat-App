...
const Sidebar = ({}) => {
    ...
    const {messages, users, selectedUser, getUsers, setMessages, setSelectedUser, 
        sendMessage, unseenMessages, setUnseenMessages} = useContext(ChatContext)
                    ...
                    'bg[#282142]/50'}`}> 
                        ...
                        {unseenMessages[user._id] > 0 && (
                            <p className='absolute top-3 right-3 text-xs h-5 w-5 
                                bg-violet-500/50 flex items-center justify-center 
                                text-[10px] font-semibold rounded-full'> {unseenMessages[user._id]}
                            </p>
                        )}
                    </div>
                ))}
